...
const customerTypeLabel = computed(() =>
  editForm.type ? catalogStore.getCustomerTypeName(editForm.type) : '请选择客户类型',
)
const depositStatusLabel = computed(
  () =>
    depositStatusOptions.find(([value]) => value === editForm.depositStatus)?.[1] ??
    '请选择定金状态',
)

const list = computed(() => {
  const longTermCustomers = customerStore.customers.filter((item) => item.isLongTerm !== false)
  const key = keyword.value.trim()
  if (!key) {
    return longTermCustomers
  }
  return longTermCustomers.filter((item) =>
    [item.name, item.phone, catalogStore.getCustomerTypeName(item.type)].some((field) =>
      field.includes(key),
    ),
  )
})
...
const openEdit = (id: string) => {
  const current = customerStore.getCustomerById(id)
  if (!current) {
    return
  }
  editingId.value = id
...
<template>
  <section class="bounce-in">
    <PageHeader
      ...
      right-text="新增"
      @back="router.back()"
      @right="router.push({ name: 'customer-new' })"
    />
    ...
    <div class="space-y-2">
      <article
        ...
        :key="item.id"
        class="card p-3"
        :class="editingId === item.id ? 'soft-blue' : 'soft-pink'"
      >
        <div class="flex items-center justify-between gap-2">
          <div>
            <p class="font-extrabold">
              {{ item.name }}
              <span class="chip ml-1">{{ catalogStore.getCustomerTypeName(item.type) }}</span>
            </p>
            <p class="mt-1 text-xs text-slate-500">
              {{ item.phone }} · {{ item.location || '未设置拍摄地点' }}
            </p>
          </div>
          <div class="flex gap-1">
            <button class="chip" type="button" @click="openEdit(item.id)">编辑</button>
            <button
              class="chip border-yellow-200 text-amber-700"
              type="button"
        ...
        <div
          ...
          class="mt-2 space-y-2 rounded-xl border border-blue-100 bg-white p-2"
        >
          <CellGroup inset>
    ...
    <Popup v-model:show="showTypePicker" position="bottom" round>
      <Picker
        ...
        @confirm="
          ({ selectedOptions }: any) => {
            editForm.type = selectedOptions[0]?.value || editForm.type
    ...
    <Popup v-model:show="showDepositPicker" position="bottom" round>
      <Picker
        ...
        @confirm="
          ({ selectedOptions }: any) => {
            editForm.depositStatus = selectedOptions[0]?.value || editForm.depositStatus
