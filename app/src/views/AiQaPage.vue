  ...
  <section class="ai-page bounce-in">
    ...
    <footer class="chat-input-area">
      <form class="chat-form" @submit.prevent="submitQuestion">
        <textarea v-model="question" rows="1" placeholder="输入你的摄影问题...（Enter 发送，Shift+Enter 换行）" @input="resizeInput" @keydown.enter.exact.prevent="submitQuestion" />
...
<style scoped>
.ai-page { display: flex; height: calc(100dvh - 108px); min-height: 0; flex-direction: column; gap: 12px; overflow: hidden; padding-top: 20px; }
...
@media (max-width: 430px) { .ai-page { height: calc(100dvh - 108px - env(safe-area-inset-bottom)); }.ai-header { align-items: flex-start; flex-direction: column; }.ai-actions { width: 100%; }.ai-action { flex: 1; }.message-bubble { max-width: 88%; }.knowledge-tools > * { flex: 1; padding: 0 6px; } }
</style>
