...
const monthOptions = computed(() => {
  const set = new Set(scheduleStore.schedules.map((item) => dayjs(item.date).format('YYYY-MM')))
...
const typeLabel = computed(() =>
  type.value === 'all' ? '全部类型' : catalogStore.getCustomerTypeName(type.value),
)
...
const loadHistory = async () => {
  loading.value = true
  try {
    list.value = await scheduleStore.refreshHistory(month.value, type.value)
  } finally {
    loading.value = false
  }
}
...
const resolveCustomer = (item: HistoryItem) =>
  item.customer ?? customerStore.getCustomerById(item.customerId)
</script>
    ...
    <div v-else class="space-y-2">
      ...
      >
        <p class="font-extrabold">
          ...
          {{ catalogStore.getCustomerTypeName(resolveCustomer(item)?.type ?? '') }}
        </p>
        <p class="mt-1 text-xs text-slate-600">
          {{ item.startTime }} - {{ item.endTime }} · {{ item.location }} ·
          {{ catalogStore.getServiceTypeName(item.serviceTypeCode) }} ·
          {{ depositStatusText[item.depositStatus] }}
        </p>
        <p class="mt-1 text-xs text-slate-500">备注：{{ item.note || '无' }}</p>
      </article>
      <div v-if="!list.length" class="card p-3 text-sm text-slate-500">
        该筛选条件下暂无历史排单。
      </div>
    </div>
