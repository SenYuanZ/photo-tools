...
const selectableRoleOptions = computed(() => {
  const selectedProfileRoleSet = new Set(store.profile?.roles || [])
  return store.roles
    .filter((item) => selectedProfileRoleSet.size === 0 || selectedProfileRoleSet.has(item.code))
    .map((item) => ({
      code: item.code,
      name: item.name,
      description: item.code === 'makeup_artist' ? '妆造服务' : item.code === 'photographer' ? '拍摄服务' : '角色服务',
    }))
})
...
watch(
  [selectableRoleOptions, () => store.userRole],
  ([options, userRole]) => {
    if (selectedRoleCodes.value.length) {
      selectedRoleCodes.value = selectedRoleCodes.value.filter((code) => options.some((item) => item.code === code))
      if (selectedRoleCodes.value.length) {
        return
      }
    }
...
const selectedRolePreview = computed(() =>
  selectedRoleCodes.value.map((code) => selectableRoleOptions.value.find((item) => item.code === code)?.name || code).join('、'),
)

const serviceTypeCode = computed(() => (selectedRoleCodes.value.includes('makeup_artist') ? 'makeup' : 'photography'))
  ...
  <section class="bounce-in">
    ...
    <article class="card mb-3 p-3">
      ...
      <CellGroup inset>
        ...
        <Field :model-value="selectedRolePreview || '请选择服务角色'" label="服务角色" required readonly />
      ...
      <div class="mt-3 rounded-xl border border-slate-100 bg-white p-2.5">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          >
            <i :class="selectedRoleCodes.includes(role.code) ? 'fa-solid fa-circle-check' : 'fa-regular fa-circle'" />
            {{ role.name }}
