...
<template>
  <section class="bounce-in pb-4">
    ...
    <article class="card mb-3 p-3">
      <CellGroup inset>
        <Field
          v-model="form.bookingGroupId"
          label="订单编号"
          placeholder="输入订单编号可直接查唯一订单"
          clearable
        />
        <Field
          v-model="form.modelPhone"
          label="客户手机"
          placeholder="可单独输入手机号查询"
          maxlength="11"
          clearable
        />
        <Field
      ...
      <div class="space-y-2">
        ...
        >
          ...
          <p class="mt-1 text-xs text-slate-600">
            客户：{{ item.modelName }}（{{ item.modelPhone }}）
          </p>
    ...
    <Button block round plain type="primary" @click="router.push({ name: 'login' })"
      >返回登录页</Button
    >
  </section>
</template>
