...
import { useAiQaPage } from '@/views/aiQa/hooks/useAiQaPage'
...
// The template uses this ref for scroll anchoring; keep an explicit script reference for vue-tsc.
void chatContainer
</script>
  ...
  <section class="ai-page bounce-in">
    <header class="card ai-header">
      ...
      <div class="ai-actions">
        <button type="button" class="ai-action" @click="openKnowledge">
          <i class="fa-solid fa-book-open" />知识库
        </button>
        <button type="button" class="ai-action" :disabled="isStreaming" @click="newChat">
          <i class="fa-solid fa-plus" />新会话
        </button>
      </div>
    </header>
        ...
        <div class="message-bubble" :class="{ 'assistant-bubble': message.role === 'assistant' }">
          ...
          <div v-if="message.sources?.length" class="sources">
            ...
            <button
              ...
              :key="source.chunkId || source.source"
              type="button"
              class="source-tag"
            ...
            <pre
              v-for="source in message.sources"
              v-show="expandedSource === source.source"
              :key="`${source.source}-preview`"
    ...
    <footer class="chat-input-area">
      <form class="chat-form" @submit.prevent="submitQuestion">
        ...
        />
        <button type="submit" :disabled="!question.trim() || isStreaming" aria-label="发送">
    ...
    <div v-if="showKnowledge" class="knowledge-overlay" @click.self="showKnowledge = false">
      <section class="knowledge-panel card">
        <header class="knowledge-header">
          <div>
            <h2 class="text-lg font-extrabold">知识库</h2>
            <p class="text-xs text-slate-500">
              共 {{ knowledgeDocuments.length }} 个文档 · {{ totalChunks }} 个分块 · 仅可查看
            </p>
          </div>
          <button
            type="button"
        ...
        >
          <div class="knowledge-item-top">
            <div class="min-w-0">
              <p class="truncate font-bold">{{ document.source }}</p>
              <p class="text-xs text-slate-400">
                {{ document.category || '未分类' }} · {{ document.chunkCount }} 个分块<span
                  v-if="originBadge(document)"
                  class="ml-1.5 rounded-full px-1.5 py-0.5 align-middle text-[10px] font-semibold"
                  :class="originBadge(document)!.cls"
                  >{{ originBadge(document)!.label }}</span
                >
              </p>
            </div>
            <div class="knowledge-actions">
              <button type="button" @click="viewDocument(document)">
                {{ viewingSource === document.source ? '收起' : '查看' }}
              </button>
            </div>
          </div>
    ...
    <div v-if="previewImage" class="image-preview-overlay" @click.self="previewImage = ''">
      <section class="image-preview-panel" aria-label="生成图片预览">
        ...
        <button type="button" class="image-save-button" @click="savePreviewImage">
          <i class="fa-solid fa-download" />保存图片
        </button>
      </section>
    </div>
  </section>
...
.welcome-icon {
  ...
  place-items: center;
  border-radius: 20px;
  background: linear-gradient(135deg, #ffe0ed, #fff1c9);
...
.markdown-body :deep(.generated-image) {
  display: block;
  width: min(100%, 520px);
  margin-top: 8px;
  border-radius: 14px;
...
.chat-form {
  ...
  box-shadow: 0 7px 15px var(--theme-shadow);
}
