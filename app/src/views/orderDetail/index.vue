...
import type { PublicOrderDetail } from '@/api/public-booking/types'
  ...
  <section class="bounce-in pb-4">
    <article class="card mb-3 p-4">
      <div class="flex items-center justify-between">
        ...
        <button class="chip" type="button" @click="router.push({ name: 'order-query' })">
          查询其他订单
        </button>
      </div>
    ...
    <template v-else-if="order">
      ...
      <article class="card mb-3 p-4 soft-pink">
        <p class="mb-2 text-sm font-extrabold text-slate-700">
          服务排单清单（{{ order.bookings.length }}项）
        </p>
        <div class="space-y-2">
          <div
            v-for="item in order.bookings"
            :key="item.bookingId"
            class="rounded-xl border border-rose-100 bg-white p-3"
          >
            <p class="text-xs font-bold text-slate-700">
              服务者：{{ item.providerName || '未命名服务者' }}
            </p>
            <p class="mt-1 text-xs text-slate-600">
              时段：{{ item.startTime }} - {{ item.endTime }}
            </p>
            <p class="mt-1 text-xs text-slate-600">服务类型：{{ item.serviceTypeCode }}</p>
            <p class="mt-1 text-xs text-slate-600">需求备注：{{ item.note || '无' }}</p>
            <p class="mt-1 text-xs text-slate-600">
              支付状态：{{ depositStatusText[item.depositStatus] || item.depositStatus }}（¥{{
                item.amount
              }}）
            </p>
          </div>
      ...
      <article class="card mb-3 p-3 soft-yellow text-xs text-slate-600">
        <p>
          <i
            class="fa-regular fa-image mr-1 text-amber-500"
          />建议：保持整页完整截图，便于后续核验。
        </p>
      </article>
    </template>

    <Button block round plain type="primary" @click="router.push({ name: 'model-booking' })"
      >返回模特约拍入口</Button
    >
  </section>
</template>
