...
const {
  ...
  goNextMonth,
  goCurrentMonth,
} = useScheduleCalendar(() => scheduleStore.schedules.filter((item) => item.status !== 'stored'))
  ...
  <section class="bounce-in">
    ...
    <article class="card mb-4 overflow-hidden p-3 calendar-hero">
      ...
      <div class="grid grid-cols-7 gap-1">
        ...
        >
          ...
          <span v-if="item.receivedCount" class="calendar-badge calendar-badge--received"
            >接{{ item.receivedCount }}</span
          >
          <span v-if="item.completedCount" class="calendar-badge calendar-badge--completed"
            >完{{ item.completedCount }}</span
          >
          <span v-if="item.pendingConfirmCount" class="calendar-badge calendar-badge--pending"
            >待{{ item.pendingConfirmCount }}</span
          >
        </button>
      </div>
      ...
      <div class="mt-3 flex items-center justify-between text-xs text-slate-500">
        ...
        <p class="font-extrabold text-rose-500">
          <i class="fa-solid fa-film mr-1" />{{ monthTotalLabel }}
        </p>
    ...
    <article class="mb-4 grid gap-3 sm:grid-cols-3">
      ...
      <section class="card p-3 soft-yellow">
        <header class="mb-2 flex items-center justify-between">
          <p class="font-extrabold text-amber-700">
            <i class="fa-solid fa-circle-check mr-1" />当天完成
          </p>
        ...
        <p v-else class="rounded-xl bg-white/70 px-3 py-2 text-sm text-slate-500">
          当天暂无已完成单。
        </p>
      </section>
      ...
      <section class="card p-3 soft-pink">
        <header class="mb-2 flex items-center justify-between">
          <p class="font-extrabold text-rose-600">
            <i class="fa-solid fa-hourglass-half mr-1" />待确认订单
          </p>
