...
import { Button, Cell, CellGroup, Field, Popup, Switch } from 'vant'
import PageHeader from '@/components/PageHeader.vue'
import { useInviteCodePage } from '@/views/inviteCodeManagement/hooks/useInviteCodePage'
...
<template>
  <section class="bounce-in">
    ...
    <article class="card mb-3 p-3 soft-pink">
      <div class="mb-2 flex items-center justify-between">
        <p class="text-sm font-extrabold">
          <i class="fa-solid fa-ticket mr-1 text-rose-500" />邀请码列表
        </p>
        <Button size="small" round type="primary" @click="openCreate">新增邀请码</Button>
      </div>
      <p class="text-xs text-slate-600">
        邀请码支持停用、限次与过期时间管理，注册摄影师时必须填写有效邀请码。
      </p>
    </article>
    ...
    <div v-else class="space-y-2">
      ...
      <div v-if="!listView.length" class="card p-3 text-sm text-slate-500">
        暂无邀请码，先创建一个吧。
      </div>
    </div>
    ...
    <Popup v-model:show="showEditPopup" position="bottom" round>
      <div class="p-4">
        ...
        <Button block round type="primary" class="mt-3" :loading="editing" @click="updateInviteCode"
          >保存修改</Button
        >
      </div>
    </Popup>
